...
# Obter Última Data de Atualização da Base
datas_f = df_f_curr['COMP MED'].dropna()
datas_c = df_c_curr['Comp. C'].dropna()
max_data = max([datas_f.max() if not datas_f.empty else pd.NaT, datas_c.max() if not datas_c.empty else pd.NaT])
str_ultima_atualizacao = max_data.strftime('%m/%Y') if pd.notna(max_data) else "Aguardando 1º fechamento"

# --- CABEÇALHO DINÂMICO DA PÁGINA ---
st.markdown(f"""
    <div>
        <span class="status-badge">Em andamento</span>
        <span class="date-badge">Última atualização: {str_ultima_atualizacao}</span>
    </div>
    <h1 style="margin-top: 5px; margin-bottom: 5px;">{selected_obra_str if selected_obra_str != "Todas as Obras" else "PORTFÓLIO COMPLETO DE OBRAS"}</h1>
    <div class="header-info">
        🏦 <b>Cliente:</b> Prefeitura Municipal &nbsp;&nbsp;|&nbsp;&nbsp; 📍 <b>Local:</b> Guarulhos - SP
    </div>
""", unsafe_allow_html=True)
...
if menu_principal == "Visão geral":
    ...
    if sub_aba == "Resumo":
        ...
        with col3:
            pct_avanco_fisico = (fat_bruto / val_contrato * 100) if val_contrato > 0 else 0
            st.markdown(f'<div class="stCard"><div class="metric-label">AVANÇO FÍSICO (FATURADO)</div><div class="metric-value">{pct_avanco_fisico:.1f}%</div><div class="metric-sub">↗ medições acumuladas</div></div>', unsafe_allow_html=True)
        with col4:
        ...
        # PREPARAÇÃO DOS DADOS PARA OS GRÁFICOS (MENSAL E ACUMULADO / CURVA S)
        ...
        base_contrato = val_contrato if val_contrato > 0 else (df_m['Fat_Acum'].max() if not df_m.empty else 1)
        df_m['Pct_Avanco_Fisico'] = (df_m['Fat_Acum'] / base_contrato) * 100
        df_m['Pct_Avanco_Custo'] = (df_m['Custo_Acum'] / base_contrato) * 100

        col_g_esq, col_g_dir = st.columns([1, 1])

        with col_g_esq:
            st.markdown("### Curva S (Avanço Físico vs Custo Acumulado %)")
            fig_curva_s = go.Figure()
            fig_curva_s.add_trace(go.Scatter(
                x=df_m['MesAno'], y=df_m['Pct_Avanco_Fisico'],
                mode='lines+markers', name='Avanço Físico Acum. (%)',
                line=dict(color='#2563EB', width=3)
            ))
            fig_curva_s.add_trace(go.Scatter(
                x=df_m['MesAno'], y=df_m['Pct_Avanco_Custo'],
                mode='lines+markers', name='Avanço Custo Acum. (%)',
                line=dict(color='#DC2626', width=3, dash='dash')
            ))
            fig_curva_s.update_layout(
                paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
                xaxis_title="Mês/Ano", yaxis_title="Percentual Acumulado (%)",
        ...
        with col_g_dir:
            ...
            fig_pie.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', legend=dict(orientation="h", y=-0.1))
...
# 3. DRE
    ...
    # Valores Orçados Temporários (Prontos para integração com BD_Orcamento.xlsx)
    orc_receita = val_contrato
    orc_custo_direto = val_contrato * 0.80 if val_contrato > 0 else 0
    orc_desp_indireta = val_contrato * 0.02 if val_contrato > 0 else 0
    orc_resultado = orc_receita - orc_custo_direto - orc_desp_indireta

    dre_df = pd.DataFrame({
        "Conta": ["Receita Bruta (Faturamento)", "Custos Diretos", "Despesas Indiretas (2%)", "Resultado Operacional"],
        "Orçado (R$)": [fmt_br(orc_receita), fmt_br(-orc_custo_direto), fmt_br(-orc_desp_indireta), fmt_br(orc_resultado)],
